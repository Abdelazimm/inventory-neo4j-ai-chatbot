...
export const CSVUploadModal: React.FC<CSVUploadModalProps> = ({ isOpen, onClose }) => {
  ...
  const handlePreview = async () => {
    ...
    setError(null);
    try {
      const data = await api.previewGraphCSV(mode, file);
      setPreview(data);
    } catch (err: any) {
  ...
  const handleCommit = async () => {
    ...
    setError(null);
    try {
      const result = await api.commitGraphCSV(mode, file);
      setCommitResult(result);
    } catch (err: any) {
      setError(err.response?.data?.detail || 'Failed to ingest graph CSV.');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div style={{
      ...
      <div className="glass-panel" style={{ width: '680px', maxHeight: '85vh', display: 'flex', flexDirection: 'column', padding: '24px', position: 'relative' }}>
        ...
        <h3 style={{ fontSize: '18px', fontWeight: 600, marginBottom: '6px' }}>
          Graph CSV Ingestion Pipeline
        </h3>
        <p style={{ fontSize: '13px', color: 'var(--text-secondary)', marginBottom: '20px' }}>
          Ingest Knowledge Graph Nodes (`label, id, name, properties`) or Relationships (`from_label, from_id, relationship_type, to_label, to_id`).
        </p>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 2fr', gap: '12px', marginBottom: '16px' }}>
          <div>
            ...
            >
              <option value="nodes">Graph Nodes</option>
              <option value="relationships">Graph Relationships</option>
            </select>
          </div>
        ...
        {!preview && !commitResult && (
          ...
          >
            <Upload size={16} />
            {loading ? 'Validating & Parsing...' : 'Preview Graph CSV'}
          </button>
        )}
          ...
          <div style={{ flex: 1, overflowY: 'auto', marginBottom: '16px' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '8px' }}>
              <span style={{ fontSize: '13px', color: 'var(--text-secondary)' }}>
                Found {preview.total_rows} records. Valid: {preview.is_valid ? '✅ Yes' : '❌ No'}
              </span>
            ...
            >
              <Check size={16} />
              {loading ? 'Executing Cypher Ingestion...' : `Confirm & Ingest ${preview.total_rows} Records`}
            </button>
          </div>
        )}
        ...
        {commitResult && (
          <div style={{ padding: '16px', background: 'rgba(16, 185, 129, 0.15)', border: '1px solid rgba(16, 185, 129, 0.3)', borderRadius: '8px', marginBottom: '16px' }}>
            ...
            <div style={{ fontSize: '13px', color: 'var(--text-secondary)' }}>
              Processed: {commitResult.total_processed} | Ingested: {commitResult.created_or_updated || commitResult.created} | Errors: {commitResult.errors?.length || 0}
            </div>
          </div>
        )}
      </div>
