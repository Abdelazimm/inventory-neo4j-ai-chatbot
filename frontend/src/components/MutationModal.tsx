...
import { GraphMutationPreview } from '../types';
...
export const MutationModal: React.FC<MutationModalProps> = ({ isOpen, onClose, userRole }) => {
  ...
  const [nodeLabel, setNodeLabel] = useState('Asset');
  const [nodeId, setNodeId] = useState('TAG-8888');
  const [propsJson, setPropsJson] = useState('{\n  "name": "Dell Precision 5570",\n  "cost": 2199.0,\n  "status": "Active"\n}');
  const [preview, setPreview] = useState<GraphMutationPreview | null>(null);
  ...
  const handlePreview = async () => {
    ...
    setLoading(true);
    try {
      const parsedProps = action === 'delete' ? {} : JSON.parse(propsJson);
      const data = await api.previewMutation(action, nodeLabel, nodeId || undefined, parsedProps);
      setPreview(data);
    } catch (err: any) {
      setError(err.message || err.response?.data?.detail || 'Failed to prepare graph mutation preview.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      setError(err.response?.data?.detail || 'Failed to execute graph mutation.');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div style={{
      ...
      <div className="glass-panel" style={{ width: '560px', padding: '24px', position: 'relative' }}>
        ...
        <h3 style={{ fontSize: '18px', fontWeight: 600, marginBottom: '6px' }}>
          Safe Parameterized Graph Mutation
        </h3>
        <p style={{ fontSize: '13px', color: 'var(--text-secondary)', marginBottom: '16px' }}>
          Perform authorized graph updates with 2-step verification. The LLM cannot execute raw arbitrary Cypher mutations.
        </p>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
              <div>
                ...
                >
                  <option value="create">Create Node (MERGE)</option>
                  <option value="update">Update Node Properties (SET)</option>
                  {userRole === 'admin' && <option value="delete">Delete Node (DETACH DELETE - Admin)</option>}
                </select>
              </div>

              <div>
                <label style={{ fontSize: '12px', color: 'var(--text-secondary)', display: 'block', marginBottom: '4px' }}>Node Label</label>
                <select
                  value={nodeLabel}
            ...
            <div>
              <label style={{ fontSize: '12px', color: 'var(--text-secondary)', display: 'block', marginBottom: '4px' }}>Unique Node ID (e.g. tag/code)</label>
              <input
                type="text"
                value={nodeId}
            ...
            {action !== 'delete' && (
              <div>
                <label style={{ fontSize: '12px', color: 'var(--text-secondary)', display: 'block', marginBottom: '4px' }}>Properties JSON</label>
                <textarea
                  value={propsJson}
                  onChange={(e) => setPropsJson(e.target.value)}
                  rows={4}
                  style={{ width: '100%', padding: '8px 12px', background: '#070b12', border: '1px solid var(--border-color)', borderRadius: '6px', color: '#6ee7b7', fontFamily: 'JetBrains Mono', fontSize: '12px' }}
                />
              </div>
            )}
        ...
        {preview && (
          <div style={{ padding: '16px', background: 'rgba(245, 158, 11, 0.1)', border: '1px solid rgba(245, 158, 11, 0.3)', borderRadius: '8px' }}>
            ...
            <pre style={{ background: '#070b12', padding: '8px', borderRadius: '6px', fontSize: '11px', color: '#6ee7b7', marginBottom: '16px' }}>
            ...
            <div style={{ display: 'flex', gap: '10px' }}>
              <button className="btn btn-danger" onClick={handleConfirm} disabled={loading} style={{ flex: 1 }}>
                Confirm & Execute Cypher Mutation
              </button>
